...
g++ %FLAGS% -shared -o dist\StepShellExt.dll src\shellext\dllmain.cpp src\shellext\thumbnail_provider.cpp src\shellext\preview_handler.cpp src\viewer\scene_view.cpp %ENGINE% src\shellext\shellext.def %STATIC% -lole32 -loleaut32 -luuid -lshlwapi -lshell32 -ladvapi32 -lgdi32
if errorlevel 1 goto :error

echo [2/3] Visor 3D...
windres src\viewer\viewer.rc -O coff -o build\viewer.res
if errorlevel 1 goto :error
g++ %FLAGS% -municode -o dist\stpviewer.exe src\viewer\main.cpp src\viewer\scene_view.cpp %ENGINE% build\viewer.res -mwindows %STATIC% -lcomctl32 -lshlwapi -lole32 -loleaut32 -luuid -lgdi32 -luser32 -lshell32 -lcomdlg32 -ladvapi32
if errorlevel 1 goto :error
