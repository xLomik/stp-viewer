...
set VIEWER_UI=src\viewer\ui\theme.cpp src\viewer\ui\icons.cpp src\viewer\ui\commands.cpp src\viewer\ui\tooltip.cpp src\viewer\ui\popup_menu.cpp src\viewer\ui\ribbon.cpp src\viewer\ui\status_bar.cpp
set STATIC=-static -static-libgcc -static-libstdc++

echo [1/3] Manejador de miniaturas...
g++ %FLAGS% -shared -o dist\StepShellExt.dll src\shellext\dllmain.cpp src\shellext\thumbnail_provider.cpp src\shellext\preview_handler.cpp src\viewer\scene_view.cpp src\viewer\image_view.cpp src\viewer\text_overlay.cpp src\viewer\markup_tools.cpp %VIEWER_UI% %ENGINE% src\shellext\shellext.def %STATIC% -lole32 -loleaut32 -luuid -lshlwapi -lshell32 -ladvapi32 -lgdi32 -lgdiplus
if errorlevel 1 goto :error

echo [2/3] Visor 3D...
windres src\viewer\viewer.rc -O coff -o build\viewer.res
if errorlevel 1 goto :error
g++ %FLAGS% -municode -o dist\stpviewer.exe src\viewer\main.cpp src\viewer\export.cpp src\viewer\scene_view.cpp src\viewer\image_view.cpp src\viewer\text_overlay.cpp src\viewer\markup_tools.cpp %VIEWER_UI% %ENGINE% build\viewer.res -mwindows %STATIC% -lcomctl32 -lshlwapi -lole32 -loleaut32 -luuid -lgdi32 -luser32 -lshell32 -lcomdlg32 -ladvapi32 -lgdiplus
if errorlevel 1 goto :error
